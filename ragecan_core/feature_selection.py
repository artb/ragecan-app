...
class BaseFeatureSelection:
    ...
    def objective(self, trial, X, y):
        ...
        k = trial.suggest_int(
            "n_features_to_select", self.desired_dimension[0], self.desired_dimension[1]
        )
        ...
        score = cross_val_score(
            classifier, X_selected, y, cv=CV_FOLDS, scoring=self.metric, n_jobs=1
        ).mean()
    ...
    def get_classifier(self, trial):
        ...
        elif self.execution_classifier == "rf":
            n_estimators = trial.suggest_int(
                "rf_n_estimators", RF_ESTIMATORS_FLOOR, RF_ESTIMATORS_CELING
            )
            max_depth = trial.suggest_int(
                "rf_max_depth", RF_DEPTH_FLOOR, RF_DEPTH_CELING
            )
            classifier = RandomForestClassifier(
                n_estimators=n_estimators, max_depth=max_depth
            )
            ic(
                f"RandomForest Classifier with n_estimators={n_estimators}, max_depth={max_depth}"
            )
    ...
    def optimize(self, X, y, desired_dimension, n_trials):
        ...
        ic(
            f"Optimization completed for {self.execution_classifier}. Best parameters: {study.best_params}"
        )
    ...
    def execute_selection(
        self, X_train, y_train, X_test, y_test, desired_dimension, n_trials
    ):
        ...
        end_time = time.time()
        logging.warning(
            f"Optimization time: {(end_time - start_time) / 60:.2f} minutes."
        )
        compressed_xs, fs_metrics = self.evaluate_on_test(
            X_train, y_train, X_test, y_test
        )
        opt_res = FSOptimizationResult(
            self.name,
            best_params,
            fs_metrics[0],
            fs_metrics[1],
...
class ChiSquareFeatureSelection(BaseFeatureSelection):
    ...
    def execute_selection(
        ...
        best_params = self.optimize(
            X_train_scaled, y_train, desired_dimension, n_trials
        )
        end_time = time.time()
...
class MutualInfoFeatureSelection(BaseFeatureSelection):
    def __init__(self, metric, execution_classifier="SVM"):
        super().__init__(
            metric, mutual_info_classif, execution_classifier=execution_classifier
        )
...
class MRMRFeatureSelection(BaseFeatureSelection):
    ...
    def select_features(self, X, y, n_features):
        ...
        try:
            selected_indices = mrmr_classif(X=X, y=y_series, K=n_features)
            self.selected_features = selected_indices
            return X.iloc[:, selected_indices].values
        except Exception as e:
            logging.error(f"Error in MRMR feature selection: {str(e)}")
            # TODO: Improve gamb for returning something when MRMR breaks
